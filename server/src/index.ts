...
export async function startServer(): Promise<StartedServer> {
  ...
  if (config.heartbeatSchedulerEnabled) {
    ...
    void heartbeat
      ...
      .then(async () => {
        const scanned = await heartbeat.scanQueueStuckBatch();
        if (scanned.created > 0 || scanned.escalated > 0) {
          logger.warn({ ...scanned }, "startup queue-stuck batch watchdog created review work");
        }
      })
      .then(async () => {
    ...
    setInterval(() => {
      ...
      void heartbeat
        ...
        .then(async () => {
          const scanned = await heartbeat.scanQueueStuckBatch();
          if (scanned.created > 0 || scanned.escalated > 0) {
            logger.warn({ ...scanned }, "periodic queue-stuck batch watchdog created review work");
          }
        })
        .then(async () => {
          const reviewed = await heartbeat.reconcileProductivityReviews();
